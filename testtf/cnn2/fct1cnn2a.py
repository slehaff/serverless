...
def load_model():
    model = keras.models.load_model('models/cnn2a-bmodel-shd-1npy-89-100.h5')
    return(model)
...
def DB1_infer(x1,x2):
    ...
    wrap = nn_wrap(255.0*predicted_img[0], 255.0*predicted_img[1]) # Use scanning output   
    return(predicted_img[0], predicted_img[1], wrap)
...
save1nnwrap()
save1wrap()
# model.save('models/cnn2a-bmodel-shd-1npy-89-100.h5')
cv2.imwrite('validate/'+'cnn2a-shd-1npy-89-100-0.png',
            (1.0*combotot).astype(np.uint8))
#################################################################################################
def infer():
    folder = '/home/samir/serverless/infer/scan_im_folder/'
    nom1, denom1, wrap1 = DB1_infer(folder + 'image0.png', folder + 'image2.png')
    ...
    cv2.imwrite(folder+ 'wrap1.png', wrap1)
